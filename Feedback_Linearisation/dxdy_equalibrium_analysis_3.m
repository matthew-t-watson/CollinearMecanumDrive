...
% and then vx = sin(phi) v, vy = cos(phi) v.

% That is, we required both accelerations to exactly equal that due to rotation, i.e. no other acceleration

syms vx(t) vy(t) w1(t) w3(t) dphi(t) theta_p(t) dtheta_p(t) w3 v t Dphi;

eqn = diff(vy) == modelData.substituteParameters(modelData.pv.dvy);

eqn = rhs(eqn) - lhs(eqn);
eqn = simplify(eqn,1000);

eqn = subs(eqn, [vx vy dphi], [sin(Dphi*t)*v, cos(Dphi*t)*v, Dphi]);

eqn = formula(subs(eqn, [dtheta_p, w3], [diff(theta_p,1), diff(theta_p,2)]));

eqn = subs(eqn, [v Dphi], [1 0]);
