...
step = [
    ...
    repmat([1 0 0]', [1 steps-stepIndex+MPC.nr])
    ];
...
subplot(2,2,3); plot(res.state.t, res.state.x); hold on; stairs(res.MPC.t', res.ref.x', '--'); legend('\(x\)', '\(r_x\)', 'Location', 'SouthEast'); ylim([-0.2 1.2]);
...
subplot(2,2,4); plot(res.state.t, res.state.x_dot); hold on; plot([min(res.state.t) max(res.state.t)], [MPC.v_x_max -MPC.v_x_max; MPC.v_x_max -MPC.v_x_max], 'Linestyle', '-.','Color',[0.8500, 0.3250, 0.0980]); ylim([-1.5 1.5]); legend('\(v_x\)', '\(\pm\overline{v}_x\)');
% subplot(4,2,8); stairs(res.MPC.t', res.MPC.t_exec'); legend('\(t_{\text{exec}}\)');

cleanfigure;
% matlab2tikz('D:\Google_Drive\Latex\Thesis\Figures\MPC\simulated_x_step_new.tex', 'height', '0.4\textheight', 'width', '0.95\linewidth', 'parseStrings', false)
